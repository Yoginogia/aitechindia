#!/usr/bin/env python3
"""
AITechNews Publisher v5.0 - RELIABILITY UPDATE
...
def call_ai(prompt: str) -> str:
          ...
          for attempt in range(3):
            try:
                ...
                if res.status_code == 200:
                    ...
                    backoff = min(30 * (attempt + 1), 120)
                    print(f"  [Gemini 429] {model_name} - Sleeping {backoff}s (backoff)...")
                    time.sleep(backoff)
                    continue
                elif res.status_code == 404:
                    print(f"  [Gemini 404] {model_name} - model not found, skipping")
                    break
                else:
                    print(f"  [Gemini Error {model_name}] {res.status_code}")
                    break
            except requests.exceptions.Timeout:
                print(f"  [Gemini Timeout] {model_name} attempt {attempt+1}")
                time.sleep(10)
            except Exception as e:
    ...
    if OPENROUTER_API_KEY:
        or_models = ["meta-llama/llama-3.3-70b-instruct:free", "google/gemma-2-9b-it:free", "mistralai/mistral-7b-instruct:free"]
...
def validate_article(content: str) -> bool:
    ...
    if not content or len(content.strip()) < 50:
        print(f"  ❌ REJECTED: Article content is empty or too short")
        return False
    words = len(content.split())
    if words < 200:
        print(f"  ❌ REJECTED: Only {words} words (minimum 200)")
        return False
    if words < 500:
...
def post_to_twitter(title: str, slug: str, category: str):
    ...
    try:
        ...
        print(f"  ⏭ Twitter: tweepy not installed, skipping")
    except Exception as e:
        error_str = str(e)
        if '401' in error_str or 'Unauthorized' in error_str:
            print(f"  ⏭ Twitter: Auth expired (401), skipping")
        elif '403' in error_str or 'Forbidden' in error_str:
            print(f"  ⏭ Twitter: Access forbidden (403), skipping")
        else:
            print(f"  ✗ Twitter: {e}")
...
def publish_category(category: str, today_formatted: str, today_slug: str,
    ...
    for item in news:
        ...
        try:
            ...
            time.sleep(45) # Sleep between articles to avoid rate limits
...
def main():
    print("=" * 60)
    print("=== AITechNews Publisher v5.0 - RELIABILITY UPDATE ===")
    print(f"Run time: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M')} UTC")
    print("Features: Dedup + Hinglish + Web Stories + Exponential Backoff + Multi-Model Fallback")
    print("=" * 60)
